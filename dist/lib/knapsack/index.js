module.exports = class MarketKnapsack {
    constructor() {
    }
    itemsCount() {
        const ITEMS_VALUES = Object.values(this.items);
        return {
            /**
             *   We bought them, but still can't take
             */
            pending: ITEMS_VALUES.filter(value => value === EMarketItemStatus.pending).length,
            /**
             * We are ready to take them or they are already sent
             */
            sent: ITEMS_VALUES.filter(value => value === EMarketItemStatus.sent).length
        };
    }
    ;
    add(item, data) {
